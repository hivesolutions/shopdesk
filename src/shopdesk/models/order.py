...
class Order(base.ShopdeskBase):
    ...
    s_id = appier.field(
        type=int, index=True, immutable=True, default=True, description="ID"
    )
    ...
    s_currency = appier.field(index=True, immutable=True, description="Currency")

    s_quantity = appier.field(
        type=int, index=True, immutable=True, description="Quantity"
    )

    s_created_at = appier.field(index=True, immutable=True, description="Created At")

    s_gateway = appier.field(index=True, immutable=True, description="Gateway")
    ...
    s_fulfillment = appier.field(index=True, description="Fulfillment")

    s_email = appier.field(
        index=True, immutable=True, meta="email", description="Email"
    )

    s_shipping_name = appier.field(
        index=True, immutable=True, description="Shipping Name"
    )

    s_shipping_street = appier.field(
        index=True, immutable=True, description="Shipping Street"
    )

    s_shipping_city = appier.field(
        index=True, immutable=True, description="Shipping City"
    )

    s_shipping_province = appier.field(
        index=True, immutable=True, description="Shipping Province"
    )

    s_shipping_country_code = appier.field(
        index=True, immutable=True, description="Shipping Country Code"
    )

    s_shipping_zip = appier.field(
        index=True, immutable=True, description="Shipping Zip Code"
    )

    s_billing_name = appier.field(
        index=True, immutable=True, description="Billing Name"
    )
    ...
    payment = appier.field(
        type=int, initial=0, safe=True, index=True, meta="enum", enum=PAYMENT_S
    )

    note_sent = appier.field(type=bool, safe=True, index=True)

    email_sent = appier.field(type=bool, safe=True, index=True)

    warning_sent = appier.field(type=bool, safe=True, index=True)

    confirmation_sent = appier.field(type=bool, safe=True, index=True)

    entity = appier.field(index=True)

    reference = appier.field(index=True)
    ...
    def from_shopify(cls, order, transactions=[], strict=False):
        ...
        if transactions:
            gateway = transactions[0].get("gateway", gateway)
        if strict and not gateway:
            raise appier.OperationalError(message="No gateway defined for order")
        return cls(
            s_id=order["id"],
            s_name=order["name"],
    ...
    def generate_mb(cls, absolute=False):
        return appier.get_app().url_for("order.generate_mb_json", absolute=absolute)

    @classmethod
    @appier.link(name="Export Shelve")
    def export_shelve_url(cls, absolute=False):
        return appier.get_app().url_for("admin.export_shelve", absolute=absolute)

    @classmethod
    @appier.link(name="Export CTT", context=True)
    def ctt_csv_url(cls, view=None, context=None, absolute=False):
        return appier.get_app().url_for(
            "order.ctt_csv", view=view, context=context, absolute=absolute
        )
    ...
    def checker_url(cls, view=None, context=None, absolute=False):
        return appier.get_app().url_for(
            "order.checker", view=view, context=context, absolute=absolute
        )

    @classmethod
    @appier.operation(
        name="Import Shelve", parameters=(("Shelve File", "file", "file"),)
    )
    ...
    def explorer_v(cls, s_name, s_gateway, reference_id, *args, **kwargs):
        kwargs["sort"] = kwargs.get("sort", [("s_id", -1)])
        if s_name:
            kwargs.update(s_name=s_name)
        if s_gateway:
            kwargs.update(s_gateway=s_gateway)
        if reference_id:
            kwargs.update(reference_id=reference_id)
        return appier.lazy_dict(
            model=cls,
            kwargs=kwargs,
            entities=appier.lazy(lambda: cls.find(*args, **kwargs)),
    ...
    def start_payment(self):
        ...
        if not hasattr(self, "s_status") or not self.s_status:
            return
        if not hasattr(self, "s_gateway") or not self.s_gateway:
            return
        if not self.s_gateway in Order.VALID_PAYMENTS:
            return
        if float(self.s_total_price) < Order.MIN_VALUE:
            return
        if self.s_status == "pending":
            self.payment = Order.PENDING
            return
        if self.s_status == "refunded":
            self.payment = Order.REFUNDED
            return
